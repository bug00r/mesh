...
main() 
{
	...
	Vec3 lb = {0.f, 0.f, 0.5f};
	Vec3 rt = {1.f, 1.f, 0.f};
	Vec3 lt = {0.f, 1.f, 0.5f};
	Vec3 rb = {1.f, 0.f, 0.0f};
	
	Mesh * quad = create_quad3(&lb, &rb, &lt, &rt);
	...
	Vec3 center = { .5f, .5f, .5f };
	float sidelen = 1.f;
	
	Mesh * cube = create_cube3_center(&center, sidelen);
	
	//for ( int cntshape = 0; cntshape < cube->cntShapes ; ++cntshape)
	//{
	//	printf("shape [%i]:\n",cntshape);
	//	Shape * shape = cube->shapes[cntshape];
	//	if (shape == NULL) {
	...
	Mesh * map = create_hmap_from_array(&waterfall_data[0], 30, 8);
	...
	Shape *polygon_shape = create_shape_polygon3(points, 8);

	Mesh *polygon = create_polygon3(points, 8);
	Mesh *polygon2 = create_polygon_shape(polygon_shape);
